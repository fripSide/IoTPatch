...
TIM_HandleTypeDef TIM3_Handler;      //定时器1句柄


/**
 * @brief	红外遥控初始化函数，设置IO以及TIM3_CH4的输入捕获
 *			TIM3挂载APB2上80Mhz
 *
 * @param   void
 *
...
void Reception_Init(void)
{
    ...
    TIM3_Handler.Init.Prescaler = 79;					//预分频器,1M的计数频率,1us加1.
    ...
    TIM3_CH4Config.ICFilter = 0x03;                     	//IC1F=0003 8个定时器时钟周期滤波
    HAL_TIM_IC_ConfigChannel(&TIM3_Handler, &TIM3_CH4Config, TIM_CHANNEL_4); //配置TIM3通道4
    HAL_TIM_IC_Start_IT(&TIM3_Handler, TIM_CHANNEL_4);  	//开始捕获TIM3的通道4
    __HAL_TIM_ENABLE_IT(&TIM3_Handler, TIM_IT_UPDATE);  	//使能更新中断
}


/**
 * @brief	TIM3底层驱动，时钟使能，引脚配置
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)//捕获中断发生时执行
{
    if(htim->Instance == TIM3)
    {
        if(RDATA)//上升沿捕获
        {
            ...
            TIM_SET_CAPTUREPOLARITY(&TIM3_Handler, TIM_CHANNEL_4, TIM_ICPOLARITY_FALLING); //CC1P=1 设置为下降沿捕获
        ...
        {
            Dval = HAL_TIM_ReadCapturedValue(&TIM3_Handler, TIM_CHANNEL_4); //读取CCR1也可以清CC1IF标志位
            TIM_RESET_CAPTUREPOLARITY(&TIM3_Handler, TIM_CHANNEL_4);  //一定要先清除原来的设置！！
            ...
            if(RmtSta & 0X10)					//完成一次高电平捕获
            {
                if(RmtSta & 0X80) //接收到了引导码
                {
                    ...
                    else if(Dval > 2100 && Dval < 2800)	//得到按键键值增加的信息 2500为标准值2.5ms
                    {
                        RmtCnt++; 		//按键次数增加1次
                        RmtSta &= 0XF0;	//清空计时器
                    }
                ...
                else if(Dval > 4200 && Dval < 4800)		//4500为标准值4.5ms
                {
